...
export async function runServer(opts: RunServerOptions = {}): Promise<ServerHandle> {
  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    // Prime the fast-mode badge so a freshly-loaded / reconnected client
    // shows the ⚡ state immediately instead of waiting for the next turn to
    // re-broadcast it.
    safeSend({
      kind: "fast_mode",
      snapshot: agentManager.getFastModeSnapshot(),
    } satisfies WsEnvelope);
